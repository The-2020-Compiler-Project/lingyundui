#include"中间代码优化.h"
using namespace std;
extern struct quaternary qt[100];//四元式数组
...
void creatDAG()
{
	...
	while (gettac(&tac))
	{
		...
		case 0:
			...
			break;
		case 1://2.(1)如果Node(B)是标记为常数的叶结点,转2.(2),否则转2.(3)
...
void delid(int num, string var)//删除结点的变量
{
	int i;
	for (i = 0; dag.node[num].var[i] != var; i++);
	for (i; i < dag.node[num].count; i++)
		dag.node[num].var[i] = dag.node[num].var[i + 1];
	dag.node[num].count -= 1;
}
